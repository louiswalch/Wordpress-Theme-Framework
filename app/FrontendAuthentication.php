...
class FrontendAuthentication {
	...
	public function __construct() {
        ...
        if (!HelloFrameworkConfig('frontend/login_required')) return false;
    ...
    public function addMessage() {
        ...
        if (!HelloFrameworkConfig('frontend/login_required/message')) return;
        ...
        add_filter('login_message', function() {
            ...
            if (isset($_REQUEST['redirect_to']) && (!strpos($_REQUEST['redirect_to'],'wp-admin') !== false)) {
                return '<p class="message framework-login-message">' . HelloFrameworkConfig('frontend/login_required/message') . '</p>';
            }
        });
    ...
    public function validate() {
        ...
        if (detect_environment('development') && HelloFrameworkConfig('frontend/login_required/ignore_development')) {
            return $this->_allowed();
        }

        if (detect_environment('staging') && HelloFrameworkConfig('frontend/login_required/ignore_staging')) {
            return $this->_allowed();
        }
        ...
        if (HelloFrameworkConfig('frontend/login_required/custom')) {
            $callback = HelloFrameworkConfig('frontend/login_required/custom/allow_callback');
        ...
        // if (is_user_logged_in() && count(HelloFrameworkConfig('frontend/login_required/allow_roles'))) {
        //     $roles  = HelloFrameworkConfig('frontend/login_required/allow_roles');
