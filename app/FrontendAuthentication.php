...
class FrontendAuthentication {
	...
	public function __construct() {
        ...
        if (!CONFIG('frontend/authentication')) return false;
    ...
    public function validate() {
        ...
        if (detect_environment('development') && CONFIG('frontend/authentication/ignore_development')) {
            return $this->_allowed();
        }

        if (detect_environment('staging') && CONFIG('frontend/authentication/ignore_staging')) {
            return $this->_allowed();
        }
        ...
        if (CONFIG('frontend/login_required/custom')) {
            ...
            if (is_callable($callback) && ((bool) $callback())) {
                ...
                return $this->_blocked('Custom Theme Logic');                
            }
        }

        // If you are already viewing the login page or this page has a post password assigned, we're not worried about you.
        if ($GLOBALS['pagenow'] === 'wp-login.php' || !empty($post->post_password)) {
            return $this->_allowed();
        }

        // If the user is logged in, we still need to check against allowed roles.
        if (is_user_logged_in() && count(CONFIG('frontend/authentication/allow_roles'))) {

            $roles  = CONFIG('frontend/authentication/allow_roles');
